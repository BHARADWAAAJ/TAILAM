...
(function () {
  ...
  // V1.1 — the early export bar placed right after the diagnosis (see
  // index.html 5.4). Same functions as the end-of-page card above, so
  // there is exactly one export implementation, reached from two places.
  on('btn-export-main-pdf-top',  () => exportPDF('main'));
  on('btn-export-main-xlsx-top', () => exportExcelX('main'));
  ...
  on('btn-export-oltc-pdf-top',  () => exportPDF('oltc'));
  on('btn-export-oltc-xlsx-top', () => exportExcelX('oltc'));
